...
*[2026-02-05]*: **Weaving External Patterns into Existing Content** - When integrating patterns from external .claude/ implementations (e.g., KotaDB audits, production systems), choose insertion point based on pattern weight: (1) **Inline timestamp addition** (lightweight, subsection within existing section using *[YYYY-MM-DD]*: prefix), (2) **New H2 section** (medium weight, 60-220 lines with Mental Model, subsections, When to Use, Connections, Sources), (3) **Major chapter section** (heavyweight, 300+ lines with full 12-section structure). Implementation standards: (a) Open with timestamp + source context ("Advanced .claude/ implementations use..."), (b) Show concrete syntax (JSON schemas, TypeScript code, YAML frontmatter—not abstract descriptions), (c) Include comparison tables for decision frameworks (Good fit / Poor fit bullets), (d) Add Connections subsection with bidirectional links, (e) Attribute sources specifically ("Advanced external .claude/ implementation patterns documented in KotaDB analysis"), (f) Preserve third-person voice throughout. Anti-pattern: rewriting existing sections—instead add new timestamped sections that complement without disruption. Use `---` separator before new section. Update last_updated frontmatter field. Evidence: 6-file update added Context Contracts (chapters/5-tool-use/5-skills-and-meta-tools.md, 64 lines), Agent Registry (chapters/6-patterns/2-self-improving-experts.md, 127 lines), Hook-Based Enforcement (chapters/7-practices/4-production-concerns.md, 219 lines), Flat Team Orchestration (chapters/6-patterns/8-expert-swarm-pattern.md, 160 lines), Learning Separation (chapters/7-practices/6-knowledge-evolution.md, 54 lines).
